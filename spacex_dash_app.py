...
def get_scatter_chart(entered_site, payload_mass):
    ...
    filter_df=spacex_df[filter]
    df = filter_df[filter_df['Launch Site']==str(entered_site)]
    if entered_site == 'ALL':
        fig = px.scatter(x=filter_df['Payload Mass (kg)'],y=filter_df['class'], color=filter_df['Booster Version Category'],
